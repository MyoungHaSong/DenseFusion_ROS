...
def seg_predict(image):
    ...
    try:
        with torch.no_grad():
            ...
            cv2.imwrite('./segmentation_image.png', pub_label)
...
#####################################################################################
                ...
                open_cv_image = img.copy()
                open_cv_image = cv2.cvtColor(open_cv_image, cv2.COLOR_RGB2BGR)
                imgpts, jac = cv2.projectPoints(cld[itemid], dof[0:3,0:3],dof[0:3,3],cam_mat,dist) # 13 = mug
...
def pose_server():
    ...
    s = rospy.Service('/cvipl/pose_server', AddTwoInts, implimentation_seg)
    # only Segmentation
...
def implimentation_seg(req):
    ...
    if req.a == 2 :
        print(cv_image.shape,cv_depth.shape,detect_res)
        seg_result = seg_predict(cv_image)
