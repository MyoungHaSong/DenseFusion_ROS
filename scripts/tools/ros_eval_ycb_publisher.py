...
parser.add_argument('--image_subscriber', type=str,default='/camera/color/image_raw')
parser.add_argument('--depth_subscriber', type=str,default='/camera/depth/image_rect_raw')
...
#cam_fx = 1066.778
#cam_fy = 1067.487
cam_cx = 331.52874755859375
cam_cy = 249.5271453857422
cam_fx = 610.6751708984375
cam_fy = 610.5318603515625
cam_scale = 1000.0
num_obj = 21
img_width = 480
img_length = 640
...
def pose_predict(img, depth):
    ...
    try:
        ...
        label = label-1 # to match labeling results to class list
        pub_label =pub_label * 50
        label_pub.publish(bridge.cv2_to_imgmsg(pub_label,'8UC1'))

        object_number = len(np.unique(label))
        print('unique lable: ', np.unique(label))

        my_result_wo_refine = []
        my_result = []
        open_cv_image = np.zeros_like(img)

        for idx in range(object_number):
            if idx == 0: continue

            itemid = np.unique(label)[idx]
            print('itemid: ', itemid)

            try:
                #cv2.imwrite('/root/catkin_ws/src/dnsefusion/scripts/experiments/scripts/segmentation_image.png', label)
                rmin, rmax, cmin,cmax = get_bbox(label)
                ...
                cam_mat = cv2.UMat(np.matrix([[cam_fx, 0, cam_cx], [0, cam_fy, cam_cy],
                             [0, 0, 1]]))
                imgpts, jac = cv2.projectPoints(cld[itemid], dof[0:3,0:3],dof[0:3,3],cam_mat,dist) # 14 mugcup
...
def implimentation_seg():
    ...
    pose_estimation,fit_image = pose_predict(cv_image, cv_depth)
    print('pose_estimation: ', pose_estimation)
...
def main():
    ...
    rgb_sub = rospy.Subscriber(opt.image_subscriber,Image, image_callback)
    depth_sub = rospy.Subscriber(opt.depth_subscriber,Image, depth_callback)
    rois_sub = rospy.Subscriber('/bbox',BoundingBoxes, rois_callback)
    rospy.spin()
