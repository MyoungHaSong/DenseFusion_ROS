...
parser.add_argument('--checkpoint_path', type=str, default='', required=True, help='The path to the pretrained weights of model')
...
class object_pose_estimation:
    def __init__(self):
        ...
        # depth_sub = rospy.Subscriber('',Image, depth_callback)
        # rois_sub = rospy.Subscriber('',BoundingBoxes, rois_callback)
        ...
        self.rois_sub = message_filters.Subscriber('/darknet_ros/bounding_boxes',BoundingBoxes)
        self.pose_pub = rospy.Publisher('/pose_pub', PoseArray,queue_size = 10)
        self.ts = message_filters.TimeSynchronizer([self.rgb_sub, self.depth_sub,self.rois_sub], queue_size = 10)
        self.ts.registerCallback(self.estimation_callback)
